...
This package publishes to **npm** (`@tangent.to/proba`) and **JSR**
(`@tangent/proba`) from one GitHub Actions workflow, `.github/workflows/release.yml`,
...
- The JSR job disables Deno's 24-hour dependency cooldown so a package can be
  released the same day as an intra-suite dependency. It is a no-op here (no
  intra-suite deps) but kept identical across the suite for consistency.
